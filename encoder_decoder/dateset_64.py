...
class TrainCorpos():
    def __init__(self, path, max_vocab_size = 10000, max_length_in = 32, max_length_out = 128):
        ...
        self.tokenized_corpus_x = self.ToToken(ls = self.input_ls, max_length=max_length_out, max_vocab_size=max_vocab_size)
    ...
    def ToToken(self, ls, max_length=64, max_vocab_size = 10000):
        # 添加填充标记
    ...
    def split(self):
        ...
        ls = self.tokenized_corpus_x["input_ids"].tolist()
        ...
        for i in tqdm(range(self.data_length)):
            # for _ in range(10):
            #     # 随机选一个子串
            #     end = random.randint(0, min(self.token_counts[i], self.max_length_out-1))
            #     # 保存到 x_seq_ls 和 y_seq_ls 中
            #     '''
            #     补充到128的长度
            #     '''
            #     y_seq = [50258] + tokenized_list[i][0:end] + [50257] * (self.max_length_out - end - 1)
            #     print(len(y_seq))
            #
            #     y_seq_ls.append(y_seq)
            #     x_seq_ls.append(ls[i])
            end = min(self.token_counts[i], self.max_length_out-1)
            y_seq = [50258] + tokenized_list[i][:end] + [50257] * (self.max_length_out - end - 1)
            # print(len(y_seq))
